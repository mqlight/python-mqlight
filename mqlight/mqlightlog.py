...
class MQLightLog():
    ...
    def _entry_level(self, level, name, id):
        self._write(level, ENTRY_IND[0:len(stack)], name)
        stack.append(name)
    ...
    def _exit_level(self, level, name, id, rc):
        self._write(level, EXIT_IND[0:len(stack)-1], name, id, rc)
        if len(stack) == 1:
            f.ffdc('MQLightLog._exit_level', 10, id, name)
        last = stack.pop()
        while last != name:
            if len(stack) == 1:
    ...
    def emit(self, id, event, *args):
        self._write(EMIT, id, event, args)

    def error(self, id, *args):
        self._write(ERROR, id, args)
